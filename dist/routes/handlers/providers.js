...
exports.newUserAccidentHandler = async (req, res) => {
    ...
    user_1.UserModel.findOneAndUpdate({ _id: ObjectId(uid) }, { $push: { accidents: req.body } }, { runValidators: true, new: true })
        .then((user) => {
...
exports.updateUserAccidentHandler = (req, res) => {
    ...
    const id = req.param('id');
    req.body._id = id;
    user_1.UserModel.findOneAndUpdate({ _id: ObjectId(uid), 'accidents._id': ObjectId(id) }, { $set: { 'accidents.$': req.body } }, { runValidators: true, new: true })
        .then((user) => {
        console.log(user);
        if (user) {
