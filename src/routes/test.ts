...
import { sendTestMail } from '../providers/sendgrid/sendgrid';
...
export const testRouter = express.Router()
    ...
    // test (mail send)
    .get('/up', async (req: Request & { tokenContent?: any }, res: Response) => {
        ...
        // 
        // uploadImages(imgs).then(_ => console.log(_))
        const urls = await uploadImagesBase64Async(images)
        console.log(urls)
        images = images.map((value, i) => {
            value.imageUrl = urls[i] // change base64 by url
            return value
        })
        console.log(images)
        res.json(images)
    ...
    // test (image upload)
    .get('/uploadimage', (req: Request & { tokenContent?: any }, res: Response) => {

    })

    // test (simulate register)
    .get('/register', (req: Request & { tokenContent?: any }, res: Response) => {
    ...
    // test (import insurances)
    .get('/insurances', (req: Request & { tokenContent?: any }, res: Response) => {

        // command mongo shell to delete all entries
        // db.insurances.remove({})
    ...
    // test (mail send)
    .get('/mail', (req: Request & { tokenContent?: any }, res: Response) => {
        sendTestMail()
        res.json({ ok: 200 })
    })
